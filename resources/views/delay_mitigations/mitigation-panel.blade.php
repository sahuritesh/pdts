    ...
    <div class="col-md-12">
        ...
        <div class="text-end mb-2">
            <a href="javascript:void(0)"
               onclick="openSideLayout({}, '{{ $addUrl }}', 'Add Mitigation', 85); return false;"
               class="btn btn-primary btn-sm waves-effect waves-light">
                <i class="fas fa-plus fa-fw"></i> Add Mitigation
            </a>
        </div>
        <div class="tablemain">
            <table id="mitigationPanelTable" class="table table-bordered nowrap datatable" style="width:100%;">
                <thead>
                    <tr>
...
(function() {
    ...
    var dataUrl = "{{ getProjectUrl('get_delay_mitigation_list') }}";

    window.reloadMitigationPanel = function(url) {
    ...
    window.openMitigationEdit = function(editUrl) {
        openSideLayout({}, editUrl, 'Edit Mitigation', 85);
    };

    $('.sidelayoutTitle').html('{{ $pageTitle ?? "Mitigations" }}');

    if ($.fn.DataTable && !$.fn.DataTable.isDataTable('#mitigationPanelTable')) {
        $('#mitigationPanelTable').DataTable({
            processing: true,
            serverSide: true,
            searching: false,
            lengthChange: true,
